...
def test_tfg_flow_mu_branch_fires_without_rho():
    torch.manual_seed(0)
    pipe = _make_stub_pipe("cpu")
    B, C, H, W = pipe._test_shape
    ...
    guidance = TFGFlowGuidance(
        predictor=_StubPredictor(),
        guidance_scale=0.0,   # rho off
        mu=0.5, mu_steps=2,   # mu on
        recurrence_steps=1,
        apply_every_n_steps=1,
        apply_range=(0.0, 1.0),
    )
    guidance.set_prompt_context(
        PromptContext(prompt_embeds=torch.randn(B, 77, 4096),
                      pooled_prompt_embeds=torch.randn(B, 2048),
                      prompt_text=["test"])
    )
    sample = torch.randn(B, C, H, W)
    guided = guidance.apply(torch.randn(B, C, H, W), torch.tensor(500.0), sample, pipe)
    assert guided.shape == sample.shape
    assert not torch.allclose(guided, sample, atol=1e-6), "mu-only guidance must modify x_t"
    assert guidance.fires_this_run == 1


def test_tfg_flow_rho_plus_mu_differs_from_rho_only():
    torch.manual_seed(0)
    pipe = _make_stub_pipe("cpu")
    B, C, H, W = pipe._test_shape
    ctx = PromptContext(prompt_embeds=torch.randn(B, 77, 4096),
                        pooled_prompt_embeds=torch.randn(B, 2048),
                        prompt_text=["test"])
    sample = torch.randn(B, C, H, W)
    vel = torch.randn(B, C, H, W)

    outs = {}
    for name, mu in [("rho_only", 0.0), ("rho_mu", 0.5)]:
        g = TFGFlowGuidance(predictor=_StubPredictor(), guidance_scale=0.5,
                            mu=mu, apply_every_n_steps=1, apply_range=(0.0, 1.0))
        g.set_prompt_context(ctx)
        outs[name] = g.apply(vel.clone(), torch.tensor(500.0), sample.clone(), pipe)
    assert not torch.allclose(outs["rho_only"], outs["rho_mu"], atol=1e-6), (
        "adding mu must change the guided sample"
    )
