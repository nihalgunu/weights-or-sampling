...
class TFGFlowGuidance:
    def __init__(
        ...
        mu: strength of upstream TFG's second branch — gradient ascent on the
            x0 ESTIMATE (backprop through VAE decode + predictor only, not the
            transformer). The x0 shift is applied to x_t (x_t = x0 + sigma*v with
            v held fixed). Same gradient-norm rescale as rho, so mu is on the
            same scale as rho. 0 disables.
        mu_steps: refinement iterations per fire for the mu branch.
        """
        self.predictor = predictor
        self.rho = float(guidance_scale)
        self.mu = float(mu)
        self.mu_steps = int(mu_steps)
    ...
    def apply(
        ...
        for _ in range(self.recurrence_steps):
            t_in = timestep.expand(guided_sample.shape[0]) if timestep.dim() == 0 else timestep
            grad_x = None
            if self.rho != 0.0:
                ...
                x0_base = x0_pred.detach()
            else:
                # mu-only fire: forward pass without transformer backprop.
                with torch.no_grad():
                    x_g = guided_sample.detach().to(device=exec_dev, dtype=torch.float32)
                    v_g = transformer(
                        hidden_states=x_g.to(device=exec_dev, dtype=transformer.dtype),
                        timestep=t_in.to(exec_dev),
                        encoder_hidden_states=ctx.prompt_embeds.to(device=exec_dev, dtype=transformer.dtype),
                    ...
                    x0_base = x_g - sigma.to(exec_dev) * v_g
            ...
            if self.mu != 0.0:
                # mu branch: gradient ascent on the x0 estimate (backprop through
                # VAE decode + predictor only), then shift x_t by the x0 delta
                # (x_t = x0 + sigma*v with v held fixed).
                x0_cur = x0_base
                for _ in range(self.mu_steps):
                    with torch.enable_grad():
                        x0_var = x0_cur.detach().requires_grad_(True)
                        img_mu = vae.decode(
                            (x0_var / vae.config.scaling_factor + vae.config.shift_factor
                             ).to(device=exec_dev, dtype=vae.dtype),
                            return_dict=False,
                        )[0].to(torch.float32)
                        sc = self.predictor(img_mu, ctx.prompt_text)
                        if sc.dim() == 0:
                            sc = sc.unsqueeze(0)
                        g0 = torch.autograd.grad(sc.sum(), x0_var, create_graph=False)[0]
                    x0_cur = x0_cur + self.mu * self._rescale(g0)
                guided_sample = guided_sample + (x0_cur - x0_base).to(guided_sample.dtype)
