...
#
# Usage: HF_TOKEN=... bash scripts/cloud/run_flowgrpo_pickscore.sh [WALL_HOURS]
...
DATASET_DIR="$FLOW_GRPO_DIR/dataset/ddrl_pickscore"
mkdir -p "$DATASET_DIR"
...
# --- Append pickscore_sd3_1gpu config to flow_grpo ---
...
if ! grep -q "def pickscore_sd3_1gpu" "$CONFIG_FILE"; then
    log "Appending pickscore_sd3_1gpu config..."
...
def pickscore_sd3_1gpu():
    ...
    config.dataset = os.path.join(os.getcwd(), "dataset/ddrl_pickscore")
    config.save_dir = "logs/pickscore/sd3.5-M-1gpu-lora"
...
mkdir -p "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_pickscore"
...
timeout --signal=TERM "${WALL_SECONDS}s" \
    accelerate launch \
        ...
        --config config/grpo.py:pickscore_sd3_1gpu \
    2>&1 | tee "$PROJECT_ROOT/logs/flowgrpo_pickscore_train.log"
RC=$?
set -e
...
if [ $RC -eq 124 ]; then
    ...
    log "WARN: training exited rc=$RC. Check logs/flowgrpo_pickscore_train.log."
fi

CKPT_DIR="$FLOW_GRPO_DIR/logs/pickscore/sd3.5-M-1gpu-lora/checkpoints"
if [ -d "$CKPT_DIR" ]; then
    N=$(ls "$CKPT_DIR" | grep -c '^checkpoint-' || true)
    log "Run dir: $CKPT_DIR ($N checkpoints)"
    cp -r "$CKPT_DIR"/checkpoint-* "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_pickscore/" 2>/dev/null || true
    log "copied $(ls $PROJECT_ROOT/outputs/checkpoints/flowgrpo_pickscore/ | wc -l) checkpoints to outputs/"
else
    log "WARN: $CKPT_DIR missing — no checkpoints to copy"
fi

log "=== FlowGRPO+PickScore run done ==="
log "Logs: logs/flowgrpo_pickscore_train.log"
log "Checkpoints: outputs/checkpoints/flowgrpo_pickscore/"
