...
# Usage: bash scripts/cloud/babysit_rhoext.sh
...
INST_FILE=".lambda_instance_rhoext"; IP_FILE=".lambda_ip_rhoext"
mkdir -p logs outputs
LOG="logs/babysit_rhoext.log"
log() { echo "[$(date +%H:%M:%S)][rhoext] $*" | tee -a "$LOG"; }
...
$SSH "ubuntu@$IP" "cd ~/haotian_research-1 && mkdir -p logs && \
    setsid nohup env HF_TOKEN='$HF_TOKEN' bash scripts/cloud/run_rhoext.sh \
    > logs/rhoext_runner.log 2>&1 < /dev/null & echo kicked" \
    || { log "FATAL: kick failed"; exit 1; }
sleep 20
ALIVE=$($SSH "ubuntu@$IP" "pgrep -f run_rhoext.sh | head -1" 2>/dev/null || true)
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    sleep 120
    TAIL=$($SSH "ubuntu@$IP" "tail -6 ~/haotian_research-1/logs/rhoext_runner.log 2>/dev/null" 2>/dev/null || true)
    if echo "$TAIL" | grep -qF "=== RHOEXT done ==="; then log "runner finished."; FINISHED=1; break; fi
    if echo "$TAIL" | grep -qE "FATAL|Traceback"; then log "runner ERROR:"; echo "$TAIL" | tee -a "$LOG"; break; fi
    HB=$($SSH "ubuntu@$IP" "tail -1 ~/haotian_research-1/logs/rhoext_runner.log 2>/dev/null | tr -d '\r' | head -c 100" 2>/dev/null || echo unreachable)
    log "hb: $HB"
done

log "pulling..."
rsync -az -e "$SSH" "ubuntu@$IP:/home/ubuntu/haotian_research-1/outputs/study_pickscore_rhoext80_matrix.json" ./outputs/ >> "$LOG" 2>&1 || log "WARN: JSON pull failed"
rsync -az -e "$SSH" --include='*/' --include='/*/0/00[0-5].png' --exclude='*' \
    "ubuntu@$IP:/home/ubuntu/haotian_research-1/outputs/study_rhoext80_images/" \
    "outputs/study_rhoext80_images/" >> "$LOG" 2>&1 || true
rsync -az -e "$SSH" "ubuntu@$IP:/home/ubuntu/haotian_research-1/logs/rhoext_runner.log" ./logs/ >> "$LOG" 2>&1 || true

terminate_box
trap - EXIT
log "=== rhoext driver done ==="
