...
#
# Usage: HF_TOKEN=... bash scripts/cloud/run_flowgrpo_imagereward.sh [WALL_HOURS]
...
DATASET_DIR="$FLOW_GRPO_DIR/dataset/ddrl_imagereward"
mkdir -p "$DATASET_DIR"
...
# --- Append imagereward_sd3_1gpu config to flow_grpo ---
CONFIG_FILE="$FLOW_GRPO_DIR/config/grpo.py"
python3 - "$CONFIG_FILE" <<'STRIP_PY'
import re, sys, pathlib
p = pathlib.Path(sys.argv[1]); src = p.read_text()
new = re.sub(
    r"\n\ndef imagereward_sd3_1gpu\([^)]*\):.*?(?=\n\ndef |\Z)",
    "", src, flags=re.DOTALL,
)
if new != src: p.write_text(new)
STRIP_PY

if ! grep -q "def imagereward_sd3_1gpu" "$CONFIG_FILE"; then
    log "Appending imagereward_sd3_1gpu config..."
...
def imagereward_sd3_1gpu():
    ...
    config.reward_fn = {"imagereward": 1.0}
    # Same DDRL prompt set written by runner — apples-to-apples with DDRL CLIP 0%
    # and the prior FlowGRPO+CLIP+DDRL Δ=+0.015.
    config.dataset = os.path.join(os.getcwd(), "dataset/ddrl_imagereward")
    config.save_dir = "logs/imagereward/sd3.5-M-1gpu-lora"
...
mkdir -p "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_imagereward"
...
timeout --signal=TERM "${WALL_SECONDS}s" \
    accelerate launch \
        ...
        --config config/grpo.py:imagereward_sd3_1gpu \
    2>&1 | tee "$PROJECT_ROOT/logs/flowgrpo_imagereward_train.log"
RC=$?
set -e
...
if [ $RC -eq 124 ]; then
    ...
    log "WARN: training exited rc=$RC. Check logs/flowgrpo_imagereward_train.log."
fi

CKPT_DIR="$FLOW_GRPO_DIR/logs/imagereward/sd3.5-M-1gpu-lora/checkpoints"
if [ -d "$CKPT_DIR" ]; then
    N=$(ls "$CKPT_DIR" | grep -c '^checkpoint-' || true)
    log "Run dir: $CKPT_DIR ($N checkpoints)"
    cp -r "$CKPT_DIR"/checkpoint-* "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_imagereward/" 2>/dev/null || true
    log "copied $(ls $PROJECT_ROOT/outputs/checkpoints/flowgrpo_imagereward/ | wc -l) checkpoints to outputs/"
else
    log "WARN: $CKPT_DIR missing — no checkpoints to copy"
fi

log "=== FlowGRPO+PickScore run done ==="
log "Logs: logs/flowgrpo_imagereward_train.log"
log "Checkpoints: outputs/checkpoints/flowgrpo_imagereward/"
