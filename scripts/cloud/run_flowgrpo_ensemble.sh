#!/bin/bash
# FlowGRPO+Ensemble(0.5 CLIP + 0.5 PickScore) on the same DDRL prompt set.
#
...
#
# Usage: HF_TOKEN=... bash scripts/cloud/run_flowgrpo_ensemble.sh [WALL_HOURS]
...
log "=== FlowGRPO+Ensemble(0.5 CLIP + 0.5 PickScore) run starting (budget: ${WALL_HOURS}h / ${WALL_SECONDS}s) ==="
...
DATASET_DIR="$FLOW_GRPO_DIR/dataset/ddrl_ensemble"
mkdir -p "$DATASET_DIR"
...
# --- Append ensemble_sd3_1gpu config to flow_grpo ---
CONFIG_FILE="$FLOW_GRPO_DIR/config/grpo.py"
python3 - "$CONFIG_FILE" <<'STRIP_PY'
import re, sys, pathlib
p = pathlib.Path(sys.argv[1]); src = p.read_text()
new = re.sub(
    r"\n\ndef ensemble_sd3_1gpu\([^)]*\):.*?(?=\n\ndef |\Z)",
    "", src, flags=re.DOTALL,
)
if new != src: p.write_text(new)
STRIP_PY

if ! grep -q "def ensemble_sd3_1gpu" "$CONFIG_FILE"; then
    log "Appending ensemble_sd3_1gpu config..."
...
def ensemble_sd3_1gpu():
    ...
    config.reward_fn = {"clipscore": 0.5, "pickscore": 0.5}
    # Same DDRL prompt set written by runner — apples-to-apples with DDRL CLIP 0%
    # and the prior FlowGRPO+CLIP+DDRL Δ=+0.015.
    config.dataset = os.path.join(os.getcwd(), "dataset/ddrl_ensemble")
    config.save_dir = "logs/ensemble/sd3.5-M-1gpu-lora"
...
log "Launching FlowGRPO+Ensemble(0.5 CLIP + 0.5 PickScore) training (timeout ${WALL_SECONDS}s)..."
mkdir -p "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_ensemble"
...
timeout --signal=TERM "${WALL_SECONDS}s" \
    accelerate launch \
        ...
        --config config/grpo.py:ensemble_sd3_1gpu \
    2>&1 | tee "$PROJECT_ROOT/logs/flowgrpo_ensemble_train.log"
RC=$?
set -e
...
if [ $RC -eq 124 ]; then
    ...
    log "WARN: training exited rc=$RC. Check logs/flowgrpo_ensemble_train.log."
fi

CKPT_DIR="$FLOW_GRPO_DIR/logs/ensemble/sd3.5-M-1gpu-lora/checkpoints"
if [ -d "$CKPT_DIR" ]; then
    N=$(ls "$CKPT_DIR" | grep -c '^checkpoint-' || true)
    log "Run dir: $CKPT_DIR ($N checkpoints)"
    cp -r "$CKPT_DIR"/checkpoint-* "$PROJECT_ROOT/outputs/checkpoints/flowgrpo_ensemble/" 2>/dev/null || true
    log "copied $(ls $PROJECT_ROOT/outputs/checkpoints/flowgrpo_ensemble/ | wc -l) checkpoints to outputs/"
else
    log "WARN: $CKPT_DIR missing — no checkpoints to copy"
fi

log "=== FlowGRPO+Ensemble(0.5 CLIP + 0.5 PickScore) run done ==="
log "Logs: logs/flowgrpo_ensemble_train.log"
log "Checkpoints: outputs/checkpoints/flowgrpo_ensemble/"
