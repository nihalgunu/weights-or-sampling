#!/bin/bash
# Local watcher for FlowGRPO+CLIP+OCR run. Fixes vs prior babysit:
...
#
# Usage: bash scripts/cloud/babysit_flowgrpo_clip_ocr.sh
...
mkdir -p logs outputs/checkpoints/flowgrpo_clip_ocr
LOG=logs/babysit_clip_ocr.log
log() { echo "[$(date +%H:%M:%S)] $*" | tee -a "$LOG"; }

log "=== babysit_clip_ocr started: ip=$IP instance=$INST ==="

DEADLINE=$(( $(date +%s) + 6*3600 ))
DONE_SENTINEL="=== FlowGRPO+CLIP+OCR run done ==="

while [ "$(date +%s)" -lt "$DEADLINE" ]; do
    sleep 90
    TAIL=$($SSH "ubuntu@$IP" 'tail -10 ~/haotian_research-1/logs/flowgrpo_clip_ocr_runner.log 2>/dev/null' 2>/dev/null || true)
    if echo "$TAIL" | grep -q "$DONE_SENTINEL"; then
        log "Runner finished."
        break
    fi
    HEART=$($SSH "ubuntu@$IP" 'tail -1 ~/haotian_research-1/logs/flowgrpo_clip_ocr_runner.log 2>/dev/null | tr -d "\r" | head -c 100; echo; nvidia-smi --query-gpu=utilization.gpu,memory.used --format=csv,noheader' 2>/dev/null || echo "ssh unreachable")
...
log "Running offline CLIP-OCR eval on the box..."
$SSH "ubuntu@$IP" "HF_TOKEN=$HF_TOKEN bash ~/haotian_research-1/scripts/cloud/run_flowgrpo_clip_ocr_eval.sh" 2>&1 | tee -a "$LOG" || log "WARN: eval failed"

# Pull the eval JSON.
log "Pulling eval result..."
rsync -az -e "ssh -i $HOME/.ssh/lambda_claude -o StrictHostKeyChecking=no" \
    "ubuntu@$IP:/home/ubuntu/haotian_research-1/outputs/flowgrpo_clip_ocr_eval.json" \
    ./outputs/ 2>&1 | tee -a "$LOG" || log "WARN: eval rsync failed"

# Pull ONLY the endpoint checkpoints the eval used (not all 300+).
ENDPOINTS=$(python3 -c "
import json
d = json.load(open('outputs/flowgrpo_clip_ocr_eval.json'))
...
if [ -n "$ENDPOINTS" ]; then
    ...
    for ep in $ENDPOINTS; do
        rsync -az -e "ssh -i $HOME/.ssh/lambda_claude -o StrictHostKeyChecking=no" \
            "ubuntu@$IP:/home/ubuntu/haotian_research-1/outputs/checkpoints/flowgrpo_clip_ocr/$ep/" \
            "./outputs/checkpoints/flowgrpo_clip_ocr/$ep/" 2>&1 | tee -a "$LOG" || log "WARN: $ep rsync failed"
    done
fi
